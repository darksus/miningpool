...
log = miningpool.lib.logger.get_logger('bitcoin_rpc_manager')

from miningpool.lib.bitcoin_rpc import BitcoinRPC
...
class BitcoinRPCManager(object):
    ...
    def __init__(self):
        ...
        self.conns = {}
        self.conns[0] = BitcoinRPC(settings.COIND_RPC_HOST,
                                 settings.COIND_RPC_PORT,
                                 settings.COIND_RPC_USER,
                                 settings.COIND_RPC_PASS)
    ...
    def getnewaddress(self):
        while True:
            try:
                return self.conns[self.curr_conn].getnewaddress()
            except Exception as e:
                print "getnewaddress", e
                self.next_connection()
