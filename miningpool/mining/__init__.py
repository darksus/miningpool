...
import threading
from miningpool.mining.work_log_pruner import WorkLogPruner
...
def setup(on_startup):
    ...
    *before* you call setup() in the launcher script.'''
    ...
    log = miningpool.lib.logger.get_logger('mining')

    from interfaces import Interfaces

    from miningpool.lib.block_updater import BlockUpdater
    from miningpool.lib.template_registry import TemplateRegistry
    from miningpool.lib.bitcoin_rpc_manager import BitcoinRPCManager
    from miningpool.lib.block_template import BlockTemplate
    from miningpool.lib.coinbaser import SimpleCoinbaser
    ...
    try:
        address = getattr(settings, 'COINBASE_ADDRESS')
    except:
        address = (yield bitcoin_rpc.getnewaddress())

    coinbaser = SimpleCoinbaser(bitcoin_rpc, address)
